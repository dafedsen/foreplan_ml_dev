...
def run_model(id_user,dbase, dbset, ex_id):
    try:
        ...
        logging_ml(id_user, id_prj, id_version, id_cust, "Exponential Smoothing", "RUNNING", "Model is running", "exponential_smoothing_gpu.py : run_model", execution_id=ex_id)
        ...
        logging_ml(id_user, id_prj, id_version, id_cust, "Exponential Smoothing", "FINISHED", "Finished running model", "exponential_smoothing_gpu.py : run_model",
        ...
        logging_ml(id_user, id_prj, id_version, id_cust, "Exponential Smoothing", "ERROR", "Error in running model", "exponential_smoothing_gpu.py : run_model : " + str(e), execution_id=ex_id)
...
def predict_model(df, st, t_forecast):
    ...
    try:
        ...
        print('ERROR in exponential_smoothing_gpu.predict_model : ', e)
        pred['date'] = t_forecast['date']
        pred[level2] = 0
        pred['level1'] = level1
        pred['adj_include'] = ADJUSTMENT
